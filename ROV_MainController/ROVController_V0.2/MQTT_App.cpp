...
static uint8_t motorDir_X = MOVE_STOP;
static int16_t motorSpd_X = 0;

static uint8_t motorDir_Y = MOVE_STOP;
static int16_t motorSpd_Y = 0;
...
Adafruit_MQTT_Subscribe motorX_ctrl_dir_sub = Adafruit_MQTT_Subscribe(&mqtt,MOTOR_X_CTRL_DIR_TOPIC);
Adafruit_MQTT_Subscribe motorX_ctrl_spd_sub = Adafruit_MQTT_Subscribe(&mqtt,MOTOR_X_CTRL_SPD_TOPIC);

Adafruit_MQTT_Subscribe motorY_ctrl_dir_sub = Adafruit_MQTT_Subscribe(&mqtt,MOTOR_Y_CTRL_DIR_TOPIC);
Adafruit_MQTT_Subscribe motorY_ctrl_spd_sub = Adafruit_MQTT_Subscribe(&mqtt,MOTOR_Y_CTRL_SPD_TOPIC);
...
uint8_t MQTT_App_MotorXDirGet(void)
{
  return motorDir_X;
}

int16_t MQTT_App_MotorXSpdGet(void)
{
  return motorSpd_X;
}


uint8_t MQTT_App_MotorYDirGet(void)
{
  return motorDir_Y;
}

int16_t MQTT_App_MotorYSpdGet(void)
{
  return motorSpd_Y;
}
...
uint8_t  MQTT_SubscrbersWaitAndRead(void)
{
  ...
  {
     ret = 1;
    if (subscription == &motorX_ctrl_dir_sub)
    {
      #if MQTT_APP_SERIAL_DEBUG
      SERIAL_DEBUG.print(F("MQTT_App-> Motor Dir Got: "));
      SERIAL_DEBUG.println((char *)motorX_ctrl_dir_sub.lastread);
      #endif
      if(MOVE_FORWARD == motorX_ctrl_dir_sub.lastread[0])
      {
        motorDir_X = MOVE_FORWARD;
      }
      else if(MOVE_BACKWARD == motorX_ctrl_dir_sub.lastread[0])
      {
        motorDir_X = MOVE_BACKWARD;
      }
      else if(MOVE_RIGHT == motorX_ctrl_dir_sub.lastread[0])
      {
        motorDir_X = MOVE_RIGHT;
      }
      else if(MOVE_LEFT == motorX_ctrl_dir_sub.lastread[0])
      {
        motorDir_X = MOVE_LEFT;
      }
      else
      {
        motorDir_X = MOVE_STOP;
      }
      
    }


    if (subscription == &motorX_ctrl_spd_sub)
    {
      motorSpd_X = atoi((char *)motorX_ctrl_spd_sub.lastread);
      
      #if MQTT_APP_SERIAL_DEBUG
      SERIAL_DEBUG.print(F("MQTT_App-> Motor Speed Got: "));
      ///SERIAL_DEBUG.print((char *)motorX_ctrl_spd_sub.lastread);
      SERIAL_DEBUG.println(motorSpd_X);
      #endif
  ...
  if (subscription == &motorY_ctrl_dir_sub)
    {
      #if MQTT_APP_SERIAL_DEBUG
      SERIAL_DEBUG.print(F("MQTT_App-> Motor Dir Got: "));
      SERIAL_DEBUG.println((char *)motorY_ctrl_dir_sub.lastread);
      #endif
      if(MOVE_UP == motorY_ctrl_dir_sub.lastread[0])
      {
        motorDir_Y = MOVE_UP;
      }
      else if(MOVE_DOWN == motorY_ctrl_dir_sub.lastread[0])
      {
        motorDir_Y = MOVE_DOWN;
      }
      else
      {
        motorDir_Y = MOVE_STOP;
      }
      
    }


    if (subscription == &motorY_ctrl_spd_sub)
    {
      motorSpd_Y = atoi((char *)motorY_ctrl_spd_sub.lastread);
      
      #if MQTT_APP_SERIAL_DEBUG
      SERIAL_DEBUG.print(F("MQTT_App-> Motor Speed Got: "));
      ///SERIAL_DEBUG.print((char *)motorX_ctrl_spd_sub.lastread);
      SERIAL_DEBUG.println(motorSpd_Y);
      #endif
...
void MQTT_AddSubscribers(void)
{
  mqtt.subscribe(&motorX_ctrl_dir_sub);
  mqtt.subscribe(&motorX_ctrl_spd_sub);
  mqtt.subscribe(&motorY_ctrl_dir_sub);
  mqtt.subscribe(&motorY_ctrl_spd_sub);
